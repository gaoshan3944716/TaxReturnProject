"""PDF generator for California Form 540."""
...
class Form540Generator:
    """Generator for California Form 540 PDF."""

    def __init__(self, tax_return: TaxReturn):
        """
        Initialize the Form 540 generator.
    ...
    def _draw_header(self, c: canvas.Canvas) -> None:
        ...
        c.drawString(self.margin, self.height - self.margin,
                     f"CA Form 540 - {self.tax_return.tax_year}")
    ...
    def _draw_income_section(self, c: canvas.Canvas) -> None:
        ...
        y -= 25
        c.setFont("Helvetica", 10)
        c.drawString(self.margin, y, "5. Federal adjusted gross income")
        c.drawString(5 * inch, y, self._format_currency(self.tax_return.federal_agi))

        y -= 15
        c.drawString(self.margin, y, "6. California adjusted gross income")
        c.drawString(5 * inch, y, self._format_currency(self.tax_return.ca_agi))
    ...
    def _draw_deductions_section(self, c: canvas.Canvas) -> None:
        ...
        if self.tax_return.use_standard_deduction:
            c.drawString(self.margin, y, "7. Standard deduction")
            c.drawString(5 * inch, y, self._format_currency(
                self._get_standard_deduction()))
        else:
            c.drawString(self.margin, y, "7. Itemized deductions")
            c.drawString(5 * inch, y, self._format_currency(
                self.tax_return.itemized_deductions))

        y -= 15
        c.drawString(self.margin, y, "8. Taxable income")
        c.drawString(5 * inch, y, self._format_currency(
            self.tax_return.ca_taxable_income))

    def _draw_tax_calculations(self, c: canvas.Canvas) -> None:
        """Draw tax calculations section."""
        y = self.height - 5 * inch
        ...
        c.drawString(5 * inch, y, self._format_currency(self.tax_return.ca_tax))
        ...
        y -= 25
        c.setFont("Helvetica", 10)
        c.drawString(self.margin, y, "19. CA tax withheld")
        c.drawString(5 * inch, y, self._format_currency(
            self.tax_return.ca_tax_withheld))

        y -= 15
        c.drawString(self.margin, y, "20. SDI/VPDI withheld")
        c.drawString(5 * inch, y, self._format_currency(
            self.tax_return.ca_sdi_withheld))

        y -= 15
        total_withheld = self.tax_return.ca_tax_withheld + self.tax_return.ca_sdi_withheld
        c.drawString(self.margin, y, "21. Total payments")
        c.drawString(5 * inch, y, self._format_currency(total_withheld))
        ...
        if self.tax_return.ca_refund_or_owed > 0:
            c.drawString(self.margin, y, "28. Refund")
            c.drawString(5 * inch, y, self._format_currency(
                self.tax_return.ca_refund_or_owed))
        else:
            c.setFont("Helvetica-Bold", 11)
            c.drawString(self.margin, y, "Amount You Owe")
            y -= 25
            c.setFont("Helvetica", 10)
            c.drawString(self.margin, y, "31. Amount you owe")
            c.drawString(5 * inch, y, self._format_currency(
                abs(self.tax_return.ca_refund_or_owed)))
    ...
    def _draw_disclaimer(self, c: canvas.Canvas) -> None:
        ...
        disclaimer = (
            ...
            "For official tax filing, please use California Franchise Tax Board-approved software "
    ...
    def _get_standard_deduction(self) -> float:
        """Get CA standard deduction amount."""
        # These match the values in california_2025.json
        deductions = {
            'single': 5063,
            'married_filing_jointly': 10126,
            'married_filing_separately': 5063,
            'head_of_household': 8899,
        }
        return deductions.get(self.tax_return.filing_status.value, 0)


def generate_ca540_pdf(tax_return: TaxReturn, output_path: str) -> None:
    """
    Generate CA Form 540 PDF for a tax return.

    Args:
        tax_return: TaxReturn object with calculated tax values
        output_path: Path where the PDF will be saved
    """
    generator = Form540Generator(tax_return)
